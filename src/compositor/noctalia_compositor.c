...
static void notify_surface_scale(struct greeter_server *server,
                                 struct wlr_surface *surface,
                                 const struct greeter_output *output) {
  if (server->fractional_scale == NULL || surface == NULL || output == NULL ||
      !output->active) {
    return;
  }
...
static bool use_all_outputs(const struct greeter_server *server) {
  return server->preferred_output[0] == '\0';
}

static struct greeter_output *output_by_name(struct greeter_server *server,
                                             const char *name) {
  struct greeter_output *output;
...
static size_t collect_outputs(struct greeter_server *server,
                              struct greeter_output **out, size_t out_cap) {
  size_t count = 0;
  struct greeter_output *output;
  wl_list_for_each(output, &server->outputs, link) {
    if (count >= out_cap) {
...
static struct greeter_output *
output_for_next_view(struct greeter_server *server) {
...
static void focus_view(struct greeter_view *view) {
  ...
  if (keyboard != NULL) {
    wlr_seat_keyboard_notify_enter(server->seat, view->toplevel->base->surface,
...
static bool commit_output_enabled(struct greeter_output *output) {
  ...
  if (!output->render_initialized) {
    if (!wlr_output_init_render(output->wlr_output, server->allocator,
                                server->renderer)) {
      wlr_log(WLR_ERROR, "failed to initialize renderer for %s",
...
static bool enable_layout_output(struct greeter_output *output, int layout_x,
                                 int layout_y);

static bool layout_output_at(struct greeter_output *output, int layout_x,
                             int layout_y) {
  if (output->active) {
    struct greeter_server *server = output->server;
    if (output->layout_output != NULL) {
    ...
    output->layout_output = wlr_output_layout_add(
        server->output_layout, output->wlr_output, layout_x, layout_y);
    if (output->layout_output == NULL) {
      wlr_log(WLR_ERROR, "failed to reposition output %s in layout",
              output->wlr_output->name);
      return false;
    }
    if (output->scene_output != NULL) {
...
static bool enable_layout_output(struct greeter_output *output, int layout_x,
  ...
  output->layout_output = wlr_output_layout_add(
      server->output_layout, output->wlr_output, layout_x, layout_y);
  if (output->layout_output == NULL) {
    wlr_log(WLR_ERROR, "failed to add output %s to layout",
            output->wlr_output->name);
    return false;
  }
  ...
  wlr_scene_output_layout_add_output(
      server->scene_output_layout, output->layout_output, output->scene_output);
...
static void configure_view(struct greeter_view *view) {
  if (view == NULL || !view->mapped || view->output == NULL ||
      !view->output->active) {
    return;
  }
  ...
  wlr_output_layout_get_box(server->output_layout, output->wlr_output,
                            &layout_box);
  wlr_scene_node_set_position(&view->scene_tree->node, layout_box.x,
                              layout_box.y);
...
static bool start_child(struct greeter_server *server, char **argv,
                        bool free_argv);
...
static void schedule_launch(struct greeter_server *server) {
  ...
  server->launch_timer =
      wl_event_loop_add_timer(loop, launch_timer_fired, server);
  if (server->launch_timer != NULL) {
    wl_event_source_timer_update(server->launch_timer, 100);
  }
}
...
static void choose_outputs(struct greeter_server *server) {
  ...
  if (!use_all) {
    ...
    if (pinned == NULL) {
      wlr_log(WLR_INFO, "output '%s' not connected; using all outputs",
              server->preferred_output);
      use_all = true;
    }
  }
  ...
  if (use_all) {
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      wlr_log(WLR_INFO, "greeter output: %s at (%d,0)",
              outputs[i]->wlr_output->name, layout_x);
      int width = 0;
      int height = 0;
  ...
  } else if (pinned != NULL && !pinned->active &&
             enable_layout_output(pinned, 0, 0)) {
...
static void handle_view_map(struct wl_listener *listener, void *data) {
  ...
  view->mapped = true;
  wlr_log(WLR_INFO, "greeter surface mapped on %s",
          view->output != NULL ? view->output->wlr_output->name : "unknown");
  if (view->output != NULL) {
    notify_surface_scale(view->server, view->toplevel->base->surface,
                         view->output);
  }
  configure_view(view);
}
...
static void handle_new_toplevel(struct wl_listener *listener, void *data) {
  ...
  wlr_log(WLR_INFO, "assigning toplevel to output %s",
          output->wlr_output->name);
