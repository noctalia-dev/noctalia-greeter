...
[[nodiscard]] inline bool isUp(std::uint32_t sym) noexcept {
  return sym == XKB_KEY_Up;
}

[[nodiscard]] inline bool isDown(std::uint32_t sym) noexcept {
  return sym == XKB_KEY_Down;
}

[[nodiscard]] inline bool isEscape(std::uint32_t sym) noexcept {
  return sym == XKB_KEY_Escape;
}
