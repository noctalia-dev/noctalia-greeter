...
// Signed distance and corner flag (1 inside an active corner band).
vec2 shape_distance_with_corner(vec2 point, vec2 size, vec4 radii, vec4 corner_shapes, vec4 logical_inset) {
    ...
    // Curve AA only when the curved boundary dominates over visual_clip.
...
// Pixel-grid snap for axis-aligned edges (full inside, none outside).
...
void main() {
    ...
    // Mix fill and border rings so translucent fill does not stack on border.
