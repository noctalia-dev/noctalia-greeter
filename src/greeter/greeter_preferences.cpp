...
namespace {
  ...
  [[nodiscard]] KeyValueMap loadKeyValues(const std::filesystem::path& path) {
    ...
    while (std::getline(in, line)) {
      ...
      if (eq == std::string::npos) {
        kLog.warn("{}:{}: ignoring line without '=': '{}'", path.string(), lineNumber, stripped);
        continue;
      }
      const std::string key = trim(stripped.substr(0, eq));
      if (key.empty()) {
        kLog.warn("{}:{}: ignoring entry with empty key", path.string(), lineNumber);
        continue;
      }
      const auto value = unquoteValue(stripped.substr(eq + 1));
      if (!value.has_value()) {
        kLog.warn("{}:{}: ignoring key '{}' with empty value", path.string(), lineNumber, key);
        continue;
      }
      if (map.find(key) != map.end()) {
        kLog.warn("{}:{}: duplicate key '{}' overrides earlier value", path.string(), lineNumber, key);
      }
      map[key] = *value;
    }
    return map;
  ...
  [[nodiscard]] bool writeKeyValues(const std::filesystem::path& path, const KeyValueMap& map) {
    std::error_code ec;
  ...
  [[nodiscard]] std::optional<std::string> mapValue(const KeyValueMap& map, std::initializer_list<const char*> keys) {
...
namespace greeter {
  ...
  GreeterPreferences loadGreeterPreferences() {
    ...
    if (const auto rawScale = mapValue(map, {"scale"})) {
      if (const auto scale = parseScaleValue(*rawScale)) {
        ...
        kLog.warn("{}: invalid scale '{}' (using auto scale)", path.string(), *rawScale);
      }
    }
    return prefs;
  }
  ...
  bool installGreeterSystemLayout(const std::string_view greeterUser, std::string& errorOut) {
    ...
    kLog.info(
        "{} greeter.conf at '{}' for user '{}'", confExisted ? "updated" : "created", confPath.string(), greeterAccount
    );
    return true;
  }
